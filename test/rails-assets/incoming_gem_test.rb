...
class IncomingGemTest < Minitest::Test
  ...
  test "#new" do
    assert_raises ArgumentError do
      RailsAssets::IncomingGem.new("NOT AN IO :(")
    end
  end

  test "#valid?" do
    subject = RailsAssets::IncomingGem.new(StringIO.new('NOT A GEM'))
    refute subject.valid?

    file = File.open(GemFactory.gem_file(:example))
    subject = RailsAssets::IncomingGem.new(file)
  ...
  test "#dest_filename" do
    file = File.open(GemFactory.gem_file(:example))
    subject = RailsAssets::IncomingGem.new(file, "/root/path")
