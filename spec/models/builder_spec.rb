  ...
  context 'generates proper files in conversion', slow: true do
    ...
    def self.component(name, version = nil, opts = {}, &block)
      ...
      it "properly compile #{name} #{version} to #{gem_name}", opts[:tags] do
        ...
        basename = "rails-assets-#{version.component.name}-#{version.string}"
        @gem_root = File.join(
          Figaro.env.data_dir, 'gems', basename
        ).to_s
    ...
    def self.invalid_component(name, version = nil, opts = {})
      it "handles invalid component  #{name} #{version}", opts[:tags] do
        STDERR.puts "\n\e[34mBuilding package #{name} #{version}\e[0m"
        expect { Build::Converter.run!(name, version) }.to raise_error do |err|
          expect(err).to be_a(Build::BowerError)
          expect(err).to be_invalid
        end
      end
    end
    ...
    #
    # I'm not sure it is bug or feature, so I'm leaving it :-)
    ...
    # main is hash (this should fail)
    invalid_component 'orbicular', '1.0.3'
